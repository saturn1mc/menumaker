...
public class MMWeekMenuPdf extends Document {
	...
	public static final Font NORMAL_FONT = FontFactory.getFont(
			FontFactory.HELVETICA, 10, Font.BOLD, BaseColor.DARK_GRAY);
	...
	private void preparePdf() {
		// Week Menu paragraph
		PdfPTable weekTable = new PdfPTable(weekMenuTable.getColumnCount());
		
		for(int row = 0; row < weekMenuTable.getRowCount(); row++){
			for(int col = 0; col < weekMenuTable.getColumnCount(); col++){
				weekTable.addCell(weekMenuTable.getValueAt(row, col).toString());
			}
		}
		...
		// Shop list paragraph
		PdfPTable shopTable = new PdfPTable(SHOP_LIST_COL_COUNT);
